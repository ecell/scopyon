...
class FRAPVisualizer(LSCMVisualizer) :

	'''
	Confocal Visualization class of e-cell simulator
	'''

	def __init__(self, configs=FRAPConfigs(), effects=PhysicalEffects()) :

		assert isinstance(configs, FRAPConfigs)
	...
	def rewrite_InputData(self, output_file_dir=None) :
		...
		count_bleaching = int(round(time_bleaching/exposure_time))
