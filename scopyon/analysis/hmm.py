...
class PTHMM(_BaseHMM):
    ...
    def _check(self):
        ...
        assert self.diffusivities_.shape == (self.n_components, 1)
        self.intensity_means_ = np.asarray(self.intensity_means_)
        assert self.intensity_means_.shape == (1, 1)
        self.intensity_vars_ = np.asarray(self.intensity_vars_)
    ...
    def _generate_sample_from_state(self, state, random_state=None):
        ...
        return np.hstack([
            np.sqrt(np.power(random_state.normal(scale=np.sqrt(2 * D), size=2), 2).sum(keepdims=True)),
            random_state.normal(loc=mean, scale=np.sqrt(var), size=(1, )),
            ])
    ...
    def _init(self, X, lengths=None):
        ...
        if 'd' in self.init_params or not hasattr(self, "diffusivities_"):
            diffusivity_means = np.mean(X[:, [0]], axis=0) * 0.25
            variations = np.arange(1, self.n_components + 1)
            variations = variations / variations.sum()
            self.diffusivities_ = diffusivity_means * variations[:, np.newaxis]
        ...
        if 'm' in self.init_params or not hasattr(self, "intensity_means_"):
            ...
            # kmeans.fit(X[:, [1]])
            # self.intensity_means_ = kmeans.cluster_centers_
            self.intensity_means_ = np.array([[np.average(X[:, 1]) * 0.5]])

        if 'v' in self.init_params or not hasattr(self, "intensity_vars_"):
            var = np.var(X[:, [1]].T) + self.min_var
    ...
    def _initialize_sufficient_statistics(self):
        ...
        stats['obs2**2'] = np.zeros((self.n_components, 1))
        return stats

    def _compute_log_likelihood(self, X):
        D = self.diffusivities_
        mean = self.intensity_means_[0, 0]
        mean *= np.tile(np.arange(1, self.n_oligomers + 1), (1, self.n_components // self.n_oligomers)).T
        var = self.intensity_vars_[0, 0]
        var *= np.tile(np.arange(1, self.n_oligomers + 1), (1, self.n_components // self.n_oligomers)).T

        if any(var <= 0.0):
            raise ValueError(f'Variance must be positive [{var}]')
        q1 = np.log(X[:, [0]] / (2 * D[:, 0])) - (X[:, [0]] ** 2 / (4 * D[:, 0]))
        q2 = -0.5 * np.log(2 * np.pi * var[:, 0]) - (X[:, [1]] - mean[:, 0]) ** 2 / (2 * var[:, 0])
        # print("mean=", mean)
        # print("var=", var)
    ...
    def _accumulate_sufficient_statistics(self, stats, obs, framelogprob,
        ...
        if any(param in self.params for param in 'dmv'):
            stats['post'] += posteriors.sum(axis=0)
        if 'd' in self.params:
            stats['obs1**2'] += np.dot(posteriors.T, obs[:, [0]] ** 2)
        if 'm' in self.params:
            stats['obs2'] += np.dot(posteriors.T, obs[:, [1]])
        if 'v' in self.params:
            stats['obs2**2'] += np.dot(posteriors.T, obs[:, [1]] ** 2)
    ...
    def _do_mstep(self, stats):
        ...
        if 'm' in self.params:
            post = denom
            x = stats['obs2']
            k = np.tile(np.arange(1, self.n_oligomers + 1), (1, self.n_components // self.n_oligomers))
            self.intensity_means_ = x.sum(axis=0) / np.dot(k, post)
        ...
        if 'v' in self.params:
            ...
            k = np.tile(np.arange(1, self.n_oligomers + 1), (1, self.n_components // self.n_oligomers))
            self.intensity_vars_ = (np.dot(1 / k, x2) - 2 * mu * x.sum(axis=0) + mu ** 2 * np.dot(k, post)) / post.sum(axis=0)
